...
11. Implementar seguindo Laravel + SQL Server.
...
- usar `DB_CONNECTION=sqlsrv`;
- usar porta padrão `1433` salvo configuração diferente da Spec;
- utilizar migrations Laravel como fonte de verdade do schema;
- escolher tipos compatíveis com SQL Server, com atenção a `nvarchar`, `datetime2`, `decimal` e `uniqueidentifier`;
- não gerar SQL com sintaxe exclusiva de MySQL, como `LIMIT`;
- usar Query Builder/Eloquent ou bindings para SQL nativo;
- separar banco de Development e Test;
- validar migrations do zero contra SQL Server real/compatível, não apenas SQLite, quando a mudança depender de comportamento específico do banco;
- documentar qualquer hint, isolamento ou configuração especial de concorrência.
