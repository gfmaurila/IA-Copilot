...
2. Criar SQL Server.
3. Criar os bancos `MeuProjeto_Dev` e `MeuProjeto_Test`.
4. Gerar a senha do usuário `sa` usando a data atual no padrão:
...
5. Aplicar a mesma senha às configurações e connection strings geradas.
6. Não deixar senha fixa previamente cadastrada no template.
