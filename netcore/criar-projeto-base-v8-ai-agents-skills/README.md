# Criar Projeto Base v8 — AI Agents + Skills
...
# 🧩 Biblioteca de Skills (extensão v8)

Esta distribuição adiciona um catálogo explícito em `tasks/skills/SKILLS.md` e skills reutilizáveis de arquitetura, backend, banco, infraestrutura, qualidade, segurança e documentação.

O carregamento deve ser **sob demanda**:

```text
Task atual
   ↓
Tech Lead indica Skills
   ↓
Developer/Tester/Reviewer carrega somente as Skills indicadas
   ↓
Executa + valida
```

Isso evita enviar todo o conhecimento técnico ao modelo em cada etapa e reduz contexto desnecessário.

Consulte `tasks/skills/SKILLS.md` para o catálogo.
